...
create_start_script() {
    local server_dir="$1"
    local mem_alloc="$2"
    local mc_version="$3" # This is the Minecraft version string (e.g., "1.21.5")
...
install_new_server() {
    ...
    if [[ "$server_type" == "Modpack" ]]; then
        ...
        prompt_input "Enter the Minecraft version this modpack is for (e.g., 1.19.2, 1.20.1): " mc_version_for_modpack
    ...
    prompt_input "Enter the Minecraft version (e.g., 1.21.5, 1.19.4): " mc_version
...
update_server() {
    ...
    prompt_input "Enter the NEW Minecraft version (e.g., 1.21.5, 1.20.6). Press Enter to update to the latest build of current version: " new_version_input
...
manage_server() {
    ...
    while true; do
        ...
        echo "4) Backup World"
        echo "5) View Logs"
        echo "6) Crash Protection Settings"
        echo "7) Update Server Version"
        echo "8) Show Screen Attach Info"
        ...
        case $choice in
            ...
            8) log_info "To attach to the server console: ${C_CYAN}screen -r $session_name${C_RESET}"; sleep 3 ;;
            9) return ;; # Exit this function, returning to select_server which then goes to main_menu
            *) log_warning "Invalid option. Please enter a number between 1 and 9." ;;
        esac
