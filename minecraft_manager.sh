...
#              server properties editor. Ensures properties are set before first launch.
...
# --- [NEW] Generate an initial server.properties file ---
generate_initial_properties() {
    local server_dir="$1"
    log_info "Generating initial server.properties file..."
    cat > "$server_dir/server.properties" << EOF
#Minecraft server properties generated by script
#$(date)
...
EOF
    log_success "Created default server.properties."
}


# --- [UPDATED] Main installation process ---
install_new_server() {
    ...
    if [[ "$server_type" == "Modpack" ]]; then
        ...
        # [NEW] Generate properties and ask to edit
        if [ ! -f "$server_dir/server.properties" ]; then
             generate_initial_properties "$server_dir"
        fi
        read -p "Do you want to edit the server properties now before the first start? (y/n): " edit_now_choice
        if [[ "$edit_now_choice" == "y" || "$edit_now_choice" == "Y" ]]; then
            edit_server_properties "$server_dir"
        fi
...
# --- Helper to read a property from server.properties ---
get_property() {
    local file="$1"
    local key="$2"
...
# --- Helper to update a property in server.properties ---
update_property() {
    local file="$1"
    local key="$2"
    local value="$3"
